...
```bash
pipx install openphotos
```

(Don't have `pipx`? `sudo apt install pipx` on Debian/Ubuntu, or
`pip install --user pipx` anywhere else. `pipx` is preferred over plain
...
## Why not `apt install openphotos`?

A real `apt install` requires hosting your own Debian package repository,
which is a lot of infrastructure for a small tool like this. `pipx install`
is just as simple for you to type and run, and it's the standard way small
Python CLI tools like this get distributed.
